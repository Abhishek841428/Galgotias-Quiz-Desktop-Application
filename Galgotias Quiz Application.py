...
def loginPage(logdata):
    ...
    global login
    login = Tk()
    login.title('Galgotias Quiz Login')
    ...
    heading = Label(login_frame,text="Galgotias Quiz Login",fg="white",bg="orange")
...
def signUpPage():
    ...
    heading = Label(sup_frame,text="Galgotias Quiz SignUp",fg="#FFA500",bg="#BADA55")
    heading.config(font=('calibri 40'))
    heading.place(relx=0.1,rely=0.1)
...
def start():
    global root 
    root = Tk()
    root.title('Welcome To Galgotias Quiz')
    canvas = Canvas(root,width = 720,height = 440, bg = 'yellow')
    canvas.grid(column = 0 , row = 1)
    img = PhotoImage(file="galgotias_logo.png")
